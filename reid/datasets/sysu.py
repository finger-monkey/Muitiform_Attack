...
class Sysu(Dataset):
    ...
    def download(self):
        ...
        identities = [[[] for _ in range(6)] for _ in range(534)]#id编号最大533，但是编号是从1开始的，所以是534!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        def register(subdir, pattern=re.compile(r'([-\d]+)_c(\d)')):
            ...
            for fpath in fpaths:
                ...
                assert 0 <= pid <= 534  # pid == 0 means background
                assert 1 <= cam <= 6
                cam -= 1
                pids.add(pid)
        ...
        # assert trainval_pids.isdisjoint(gallery_pids)#这句代码的作用是检查trainval_pids和gallery_pids两个集合是否互不相交（即没有共同的元素）。如果它们互不相交，那么代码将继续执行，没有任何问题；但如果它们有共同的元素，那么将引发AssertionError异常，表示存在问题需要解决。

        # Save meta information into a json file
        meta = {'name': 'Regdb', 'shot': 'multiple', 'num_cameras': 6,

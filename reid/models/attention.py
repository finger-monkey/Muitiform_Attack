...
class IWPA(nn.Module):
    ...
    def forward(self, x, feat, t=None, part=0):
        ...
        part_feat = part_feat.permute(0, 2, 1, 3) 

        part_feat1 = self.fc1(part_feat).view(b, self.inter_channels, -1)  
        part_feat1 = part_feat1.permute(0, 2, 1)  

        part_feat2 = self.fc2(part_feat).view(b, self.inter_channels, -1)  

        part_feat3 = self.fc3(part_feat).view(b, self.inter_channels, -1)  
        part_feat3 = part_feat3.permute(0, 2, 1)   

        
        cpa_att = torch.matmul(part_feat1, part_feat2) 
        cpa_att = F.softmax(cpa_att, dim=-1)

        
        refined_part_feat = torch.matmul(cpa_att, part_feat3) 
        refined_part_feat = refined_part_feat.permute(0, 2, 1).contiguous() 
        refined_part_feat = refined_part_feat.view(b, self.inter_channels, part) 
